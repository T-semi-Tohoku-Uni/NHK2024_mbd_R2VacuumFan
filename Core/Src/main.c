...
float calculateTemperature(void) {
    ...
    float voltage = aVal * 3.3 / 4096; // ADCå€¤ã‚’é›»åœ§ã«å¤‰æ›
    float resistance = voltage / (3.3 - voltage) * R0; // é›»åœ§ã‹ã‚‰æŠµæŠ—ï¿½???????¿½?¿½??¿½?¿½???¿½?¿½??¿½?¿½????¿½?¿½??¿½?¿½???¿½?¿½??¿½?¿½?????¿½?¿½??¿½?¿½???¿½?¿½??¿½?¿½????¿½?¿½??¿½?¿½???¿½?¿½??¿½?¿½ã«å¤‰æ›
    float temperature = 1 / (1 / (273.15 + 25) + log(resistance / R1) / B); // æ¸©åº¦ã‚’è¨ˆï¿½?
    return temperature - 273.15; // æ¸©åº¦ã‚’æ‘‚æ°ã«å¤‰æ›ã—ã¦è¿”ã™
}
/* USER CODE END 0 */
